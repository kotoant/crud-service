...
create unique index order_public_id_latest_version_true_key on "order" (public_id) where latest_version = true;
--rollback drop index order_public_id_latest_version_true_key;

--changeset Anton.Kotov:CRUD-1-4
create index order_created_public_id_idx on "order" (created, public_id) where latest_version = true;
